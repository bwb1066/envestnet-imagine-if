/* ============================================================
   ...
   That swaps the mock-class selector (.env-mock → .tumi-mock), the storage-key prefix
   ('envestnet-mockup:' → 'tumi-mockup:'), and the toolbar title prefix and console label.
...
(function () {
  ...
  const STORAGE_KEY = 'envestnet-mockup:' + STORAGE_VERSION + ':' + (location.pathname.split('/').pop() || 'index.html');
  let edited = false;
  ...
  function applyEditable() {
    // Selectors target .env-mock (matches the deck's mock containers). The applyImageSwap
    ...
    const sel = [
      '.env-mock h1, .env-mock h2, .env-mock h3, .env-mock h4, .env-mock h5, .env-mock h6',
      '.env-mock p',
      '.env-mock span:not(.dot):not(.pin):not(.icon-cart)',
      '.env-mock b', '.env-mock strong', '.env-mock small', '.env-mock em',
      '.env-mock a',
      '.env-mock label', '.env-mock td', '.env-mock li',
      '.env-mock .porter-tag', '.env-mock .porter-ahead',
      '.env-mock .chip',
      '.env-mock .btn, .env-mock .btn-outline, .env-mock .btn-ghost, .env-mock .add-btn',
      '.env-mock .badge', '.env-mock .pill',
      '.env-mock .product-line', '.env-mock .product-name', '.env-mock .product-specs', '.env-mock .product-price',
      '.env-mock .name', '.env-mock .price', '.env-mock .specs', '.env-mock .desc',
      '.env-mock .num', '.env-mock .label',
      '.env-mock .when', '.env-mock .ed', '.env-mock .countdown', '.env-mock .countdown-label',
      '.env-mock .yr',
      '.env-mock .qty', '.env-mock .save',
      '.env-mock .from', '.env-mock .body-text', '.env-mock .signature',
      '.env-mock .section-eyebrow', '.env-mock .lede',
      '.env-mock .miles', '.env-mock .where', '.env-mock .date',
      '.env-mock .amount', '.env-mock .savings',
      '.mockup-caption'
    ];
  ...
  function applyImageSwap() {
    ...
    // (and some other browsers) swallow clicks because the parent .env-mock subtree may
    ...
    document.querySelectorAll('.env-mock img, .env-mock .img-swap').forEach(target => {
    ...
    document.querySelectorAll('.env-mock .img-swap *').forEach(child => {
  ...
  function injectToolbar() {
    ...
    bar.innerHTML =
      ...
      '<span class="title">' + document.title.replace(/^Sage — /, '') + '</span>' +
  ...
  console.log('%cSage editor', 'color:#1a1a1a;font-weight:700;background:#fff;padding:2px 6px',
